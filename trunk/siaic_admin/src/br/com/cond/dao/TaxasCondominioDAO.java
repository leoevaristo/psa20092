...
public class TaxasCondominioDAO {
	...
	public List<TaxasCondominio> getInadimplentes() {
		...
		String sql = "SELECT ft.COF_MES, ft.COF_ANO, ft.COF_APA_CODIGO, ft.COF_PAGO, ft.COF_DATA_VENCIMENTO, DATEDIFF(CURDATE(),COF_DATA_VENCIMENTO) AS ATRASO,  ft.COF_VALOR, ap.APA_NUMERO FROM ADMCON_TAXA_CONDOMINIO_FATOS ft, ADMCON_APARTAMENTO ap WHERE COF_PAGO = 'N' AND CURDATE() > COF_DATA_VENCIMENTO AND ft.COF_APA_CODIGO = ap.APA_CODIGO;";
		try {
