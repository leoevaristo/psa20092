...
public class InfracaoOcorrenciaBean {
	...
	public InfracaoOcorrenciaBean() {
		...
		try {
			...
			listaTodasAsInfracoes.add(new SelectItem(infracao
					.getCodigoInfracao(), label.toString()));
		} catch (Exception e) {
		}
		...
		if (listaApartamentos.isEmpty()) {
			...
			try {
				for (Apartamento ap : apartDao.getTodosOsApartamentos()) {
					...
					listaApartamentos.add(new SelectItem(ap
							.getCodigoApartamento(), label.toString()));
				}
	...
	public String addInfracaoOcorrencia() {
		...
		try {
			daoInfracaoOcorrencia
					.adicionarInfracaoOcorrencia(infracaoOcorrencia);
			r = "sucesso";
		} catch (SQLException e) {
			MensagemUtil.montarMensagemErro(e, FacesContext
					.getCurrentInstance());
			e.printStackTrace();
		}
